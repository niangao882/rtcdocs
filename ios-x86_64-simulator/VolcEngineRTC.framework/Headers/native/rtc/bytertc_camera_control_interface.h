 ...
 * @region 视频管理
 * @brief 相机控制接口
 * @note 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能使用这个接口。
 */
...
public:
     ...
     *        - < 0： 失败。
     * @note
     *        - 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能设置摄像头变焦倍数。
     ...
     * @return 最大变焦倍数
     * @note 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检测摄像头最大变焦倍数。
     */
     ...
     *        - true: 支持
     *        - false: 不支持
     * @note 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检测摄像头变焦能力。
     */
     ...
     *        - true: 支持
     *        - false: 不支持
     * @note 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检测闪光能力。
     */
     ...
     *        - < 0： 失败。
     * @note
     *        - 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能设置闪光灯。
     *        - 设置结果在调用 StopVideoCapture{@link #IRTCVideo#StopVideoCapture} 关闭内部采集后失效。
     */
     ...
     *        - true： 支持。
     *        - false： 不支持。
     * @note 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检查对焦点设置能力。
     */
     ...
     *        - < 0： 失败。
     * @note
     *        - 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集，并且使用 SDK 内部渲染时，才能设置对焦点。
     ...
     *        - true： 支持。
     *        - false： 不支持。
     * @note 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能检查曝光点设置能力。
     */
     ...
     *        - < 0： 失败。
     * @note
     *        - 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集，并且使用 SDK 内部渲染时，才能设置曝光点。
     ...
     *        - < 0： 失败。
     * @note
     *        - 必须已调用 StartVideoCapture{@link #IRTCVideo#StartVideoCapture} 使用 SDK 内部采集模块进行视频采集时，才能设置曝光补偿。
     *        - 设置结果在调用 StopVideoCapture{@link #IRTCVideo#StopVideoCapture} 关闭内部采集后失效。
     */
