...
public:
     ...
     * 等待超时后会返回空列表。超时时间默认为 10 s。建议通过 onAudioDeviceStateChanged{@link #IRTCVideoEventHandler#onAudioDeviceStateChanged} 监听到 `kMediaDeviceListUpdated` 后，再次调用本接口获取。
     * @note
     *       - 你可以在收到 onAudioDeviceStateChanged{@link #IRTCVideoEventHandler#onAudioDeviceStateChanged} 了解设备变更后，重新调用本接口以获得新的设备列表。
     ...
     *       - [205,255] 为高音量。
     * @return 方法调用结果 <br>
     *        - 0：方法调用成功。将影响 onLocalAudioPropertiesReport{@link #IRTCVideoEventHandler#onLocalAudioPropertiesReport} 回调的音量信息。
     *        - < 0：方法调用失败。
     * @note 对 Linux SDK：仅在 Client 版本有，在 Server 版本上没有。
     * @list 音频管理
     * @order 11
