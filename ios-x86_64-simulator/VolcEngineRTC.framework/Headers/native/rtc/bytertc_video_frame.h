...
enum VideoFrameType {
    ...
    /**
     * 
     * @brief CVPixelBufferRef 类型，支持 iOS and macOS 平台
     */
...
public:
    ...
    virtual uint8_t* getSupplementaryInfo(int& size) const = 0;  
    /**
     * 
     * @brief 获取本地缓冲区指针
     */
    ...
    /**
     * 
     * @brief 获取硬件加速 Context 对象(AKA Opengl Context, Vulkan Context)
     */
    ...
    /**
     * 
     * @brief 获取硬件加速 Context 的 Java 对象(Only for Android, AKA Opengl Context)
     * @return 返回 JavaLocalRef, 当不再使用时，需要手动执行 DeleteLocalRef(env, jobject)方法释放该对象
     */
...
struct ScreenVideoEncoderConfig {
    ...
    /**
     * 
     * @brief 视频编码帧率，单位：fps。建议和采集帧率保持一致。
     */
    ...
    /**
     * 
     * @brief 屏幕流编码模式。默认为清晰模式。参见 ScreenVideoEncodePreference{@link #ScreenVideoEncodePreference}。
     */
