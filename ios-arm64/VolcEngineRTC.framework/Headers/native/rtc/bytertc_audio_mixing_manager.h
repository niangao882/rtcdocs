...
public:
     ...
     * @note
     *       - 如果已经通过 preloadAudioMixing{@link #IAudioMixingManager#preloadAudioMixing} 将文件加载至内存，确保此处的 ID 与预加载时设置的 ID 相同。
     *       - 调用本方法播放音频文件后，关于当前的混音状态，会收到回调 onAudioMixingStateChanged{@link #IRTCVideoEventHandler#onaudiomixingstatechanged}。
     *       - 开始播放音频文件后，可以调用 stopAudioMixing{@link #IAudioMixingManager#stopAudioMixing} 方法停止播放音频文件。
     ...
     *        混音 ID
     * @note
     *       - 调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 方法开始播放音频文件及混音后，可以调用本方法停止播放音频文件及混音。
     *       - 调用本方法停止播放音频文件后，SDK 会向本地回调通知已停止混音，见 onAudioMixingStateChanged{@link #IRTCVideoEventHandler#onaudiomixingstatechanged}。
     ...
     * @brief 停止播放所有音频文件及混音。
     * @note
     *       - 调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 方法开始播放音频文件及混音后，可以调用本方法停止播放所有音频文件及混音。
     *       - 调用本方法停止播放所有音频文件及混音后，会收到 onAudioMixingStateChanged{@link #IRTCVideoEventHandler#onaudiomixingstatechanged} 回调，通知已停止播放和混音。
     ...
     *        混音 ID
     * @note
     *       - 调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 方法开始播放音频文件及混音后，可以通过调用本方法暂停播放音频文件及混音。
     *       - 调用本方法暂停播放音频文件及混音后，可调用 resumeAudioMixing{@link #IAudioMixingManager#resumeAudioMixing} 方法恢复播放及混音。
     ...
     * @brief 暂停播放所有音频文件及混音。
     * @note
     *       - 调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 方法开始播放音频文件及混音后，可以通过调用本方法暂停播放所有音频文件及混音。
     *       - 调用本方法暂停播放所有音频文件及混音后，可调用 resumeAllAudioMixing{@link #IAudioMixingManager#resumeAllAudioMixing} 方法恢复所有播放及混音。
     ...
     *        混音 ID
     * @note
     *       - 调用 pauseAudioMixing{@link #IAudioMixingManager#pauseAudioMixing} 方法暂停播放音频文件及混音后，可以通过调用本方法恢复播放及混音。
     ...
     * @brief 恢复播放所有音频文件及混音。
     * @note
     *       - 调用 pauseAllAudioMixing{@link #IAudioMixingManager#pauseAllAudioMixing} 方法暂停所有正在播放音频文件及混音后，可以通过调用本方法恢复播放及混音。
     ...
     *        </table>
     * @note
     *       - 本方法只是预加载指定音频文件，只有调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 方法才开始播放指定音频文件。
     *       - 调用本方法预加载音频文件后，关于当前的混音状态，会收到回调 onAudioMixingStateChanged{@link #IRTCVideoEventHandler#onaudiomixingstatechanged}。
     ...
     * @note
     *        - 调用本方法设置音频文件的声道模式前，需要先调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 开始播放音频文件。
     ...
     *        - 你需要在调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 开始混音，并且收到 onAudioMixingStateChanged{@link #IRTCVideoEventHandler#onaudiomixingstatechanged} 回调提示 AudioMixingState{@link #AudioMixingState} 状态为 `kAudioMixingStatePlaying`，AudioMixingError{@link #AudioMixingError} 错误码为 `kAudioMixingErrorOk` 之后调用该方法。
     ...
     * @note
     *       - 必须先调用本方法启动 PCM 音频数据混音，随后调用 pushAudioMixingFrame{@link #pushAudioMixingFrame} 方法，才会开始混音。
     ...
     *       - < 0: 失败
     * @note
     *      - 调用该方法前，须通过 enableAudioMixingFrame{@link #IAudioMixingManager#enableAudioMixingFrame} 启动外部音频流混音。
     ...
     *        - < 0：方法调用失败
     * @note
     *       - 调用本方法获取音频文件的音轨前，需要先调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 开始播放音频文件。
     ...
     * @note
     *       - 调用本方法设置音频文件的音轨前，需要先调用 startAudioMixing{@link #IAudioMixingManager#startAudioMixing} 开始播放音频文件。
     *       - 此方法对 enableAudioMixingFrame{@link #IAudioMixingManager#enableAudioMixingFrame} 播放的音乐无效。
     */
